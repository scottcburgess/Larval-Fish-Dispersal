### This code produces Figure S5 in the manuscript: 
...
dat <- do.call(rbind.data.frame,d_Swains_Swimming1000)
...
row.names(mp)<-coords #ycoords
 ...
 # For loop to fill in the matrix 
# j is the data frame with the model predictions
dat.passive <- do.call(rbind.data.frame,d_Swains_Passive1000)
...
plot(c(-300,300),c(-350,250),type="n",asp=1,las=1,xlab="",ylab="",cex.axis=cexl)
...
mtext("North-South distance\nfrom natal site (km)",side=2,line=3,outer=F,cex=1.1,at=-400,adj=0)
...
# dev.copy2pdf (file="Figure S5.pdf", family="ArialMT")
...
# embed_fonts ("Figure S5.pdf",outfile='Figure S5.pdf') # journals often want fonts embedded
